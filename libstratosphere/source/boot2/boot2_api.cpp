...
namespace ams::boot2 {
    ...
    namespace {
        ...
        void IterateOverFlaggedProgramsOnSdCard(F f) {
            ...
            if (R_FAILED(fs::OpenDirectory(&contents_dir, "sdmc:/ReiNX/contents", fs::OpenDirectoryMode_Directory))) {
                return;
            }
        ...
        void DetectAndDeclareFutureMitms() {
            IterateOverFlaggedProgramsOnSdCard([](ncm::ProgramId program_id) {
                ...
                /* Read the mitm list off the SD card. */
                {
                    ...
                    std::snprintf(path, sizeof(path), "sdmc:/ReiNX/contents/%016lx/mitm.lst", static_cast<u64>(program_id));
