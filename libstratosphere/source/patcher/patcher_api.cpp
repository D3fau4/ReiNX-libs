...
namespace ams::patcher
{
    ...
    static void ApplyRnxPatch(fs::FileHandle patch_file, u8 *mapped_module, size_t mapped_size)
    {
        ...
        u8 patch[0x100] = {0};
        int rxpoffset = 12;
        if (R_FAILED(fs::ReadFile(patch_file, 11, &patch_count, 1)))
        {
        }

        for (int i = 0; i < patch_count; i++)
        {
            if (R_FAILED(fs::ReadFile(patch_file, rxpoffset, &pattern_length, 1)))
            {
                return;
            }
            rxpoffset = (rxpoffset + 1);
            if (R_FAILED(fs::ReadFile(patch_file, rxpoffset, &patch_length, 1)))
            {
                return;
            }
            rxpoffset = (rxpoffset + 1);
            if (R_FAILED(fs::ReadFile(patch_file, rxpoffset, &search_multiple, 1)))
            {
                return;
            }
            rxpoffset = (rxpoffset + 1);
            if (R_FAILED(fs::ReadFile(patch_file, rxpoffset, &offset, 1)))
            {
                return;
            }
            rxpoffset = (rxpoffset + 1);
            if (R_FAILED(fs::ReadFile(patch_file, rxpoffset, pattern, pattern_length)))
            {
                return;
            }
            rxpoffset = (rxpoffset + pattern_length);
            if (R_FAILED(fs::ReadFile(patch_file, rxpoffset, patch, patch_length)))
            {
                return;
            }
        ...
        while (true)
        {
            ...
            while (true)
            {
                ...
                if (IsRXP(entry.name) == true)
                {
                    ...
                    {
                        fs::ReadFile(patch_file, 0, magic, 3);
                        fs::ReadFile(patch_file, 3, &read_id, 8);
